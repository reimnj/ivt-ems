...
// Retrieve Notice Record

$postID = $_GET['postID'];

$postQuery = "SELECT * FROM notice WHERE postID = '$postID'";
$executePostQuery = mysqli_query($connection, $postQuery);
$noticeData = mysqli_fetch_assoc($executePostQuery);
...
if(isset($_POST['update-notice'])) {
    ...
    $updateQuery = "UPDATE notice SET postAuthor='$author', postSubject='$subject', postContent='$content',  postDate='$date' WHERE postID = '$postID'";
    $executeUpdateQuery = mysqli_query($connection, $updateQuery);
    header("Location:notice_edit.php?update=success&postID=$postID");
            ...
            <div class="notice-wrapper">
                <div class="notice-content">
                    <form action="" method="post">
                        <h2>Edit Notice</h2>
                        <a href="dashboard.php">Cancel changes</a>
                        <p>
                            <?php
                                ...
                                if(isset($_GET['update'])) {
                                    echo "The notice has been updated successfully!";
                                }
                            ?>
                        </p>
                        <hr style="border: 2px solid var(--main-color); margin-bottom: 20px;">
                        <h3>Author</h3>
                        <input type="text" name="author" value="<?php echo $noticeData['postAuthor']; ?>">
                        <h3>Subject</h3>
                        <input type="text" name="subject" value="<?php echo $noticeData['postSubject']; ?>">
                        <h3>Notice Content</h3>
                        <textarea name="content" id="" cols="166" rows="20"><?php echo $noticeData['postContent']; ?></textarea>
                        <div class="notice-footer">
                            <div>
                                <h4>Date Posted</h4>
                                <input type="text" name="date" value="<?php echo $noticeData['postDate']; ?>" readonly>
                            </div>
                            <input type="submit" name="update-notice" value="Update Notice">
                        </div>
